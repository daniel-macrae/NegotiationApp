        ...
        if modelCurrentOffer == nil {
            print("modelCurrentOffer is nil")
                ...
                if let modelOffer = chunk?.slotvals["myMove"]?.description {
                    ...
                    modelCurrentOffer = Int(Float(modelOffer)!)    // I think this fixes the issue. the string modelOffer is "7.0" -> float = 7.0 -> int = 7. can't go from strign "7.0" to Int 7 I guess?
                    print("M: offer should be = " + String(Int(Float(modelOffer)!)))
